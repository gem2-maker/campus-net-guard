...
if SCRIPT_DIR.name.lower() == "scripts" and (PROJECT_ROOT / "workspace").exists():
    ROOT = PROJECT_ROOT
    ...
    if not candidates:
        for raw_line in lines:
            ...
            if len(cols) != 5:
                continue
            dest, mask, gateway, interface_ip, metric = cols
            if dest != AUTH_HOST or mask != "255.255.255.255":
                continue
            if not is_private_ip(gateway) or not is_private_ip(interface_ip):
                continue
            try:
    ...
    if not candidates:
        ...
        if ipconfig.returncode == 0:
            ...
            if ip_match and gw_match:
                ...
                if is_private_ip(iface_ip) and is_private_ip(gateway):
                    return CampusRoute(gateway=gateway, interface_ip=iface_ip)
        return None
...
def ensure_host_route(target: str, route: CampusRoute) -> None:
    ...
    if result.returncode == 0:
        ...
        return
    logging.warning("failed to add host route %s: %s", target, result.stderr.strip() or result.stdout.strip())
